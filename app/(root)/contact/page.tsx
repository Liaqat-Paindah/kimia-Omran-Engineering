...
const ContactInfoCard = ({ icon, title, content, link, index }: { icon: React.ReactNode; title: string; content: string; link?: string; index: number }) => {
  ...
  const cardContent = (
    ...
    >
      ...
      <div className="relative bg-white dark:bg-[#011b2b] border border-gray-200 dark:border-[#064e78] rounded-sm p-5 h-full">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 font-light">{content}</p>
      </div>
    </motion.div>
  );
...
const FormInput = ({ label, type, id, placeholder, required, textarea, value, onChange }: { label: string; type: string; id: string; placeholder: string; required?: boolean; textarea?: boolean; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="relative">
      <label htmlFor={id} className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">
        {label}{required && <span className="text-[#00b3aa] ml-1">*</span>}
      </label>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        >
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">Thank you for reaching out. Our team will respond within 24 hours.</p>
            <button onClick={onClose} className="relative group w-full">
              ...
              <div className="relative px-4 py-2 text-white rounded-sm font-semibold text-sm" style={{ background: colors.gradient }}>Close</div>
            </button>
          </div>
        </motion.div>
      </motion.div>
        ...
        <div className="relative container mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial="hidden" animate="visible" className="max-w-3xl mx-auto">
            <motion.div custom={0} variants={textVariants} className="inline-flex items-center gap-2 px-3 py-1 rounded-sm mb-6" style={{ background: `${colors.quinary}20`, border: `1px solid ${colors.quinary}` }}>
              <span className="text-xs font-mono" style={{ color: colors.quinary }}>GET IN TOUCH</span>
            </motion.div>
            <motion.h1 custom={1} variants={textVariants} className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6">
              Let`s Start Your <span style={{ color: colors.quinary }}>Project</span>
            </motion.h1>
            <motion.p custom={2} variants={textVariants} className="text-lg text-white/80 max-w-2xl mx-auto">
              Ready to bring your vision to life? Contact us today for a consultation and discover how Kimia Omran can make your project a reality.
            </motion.p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="py-20 bg-white dark:bg-[#010a12]">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
            <ContactInfoCard index={0} icon={<Icons.Mail />} title="EMAIL" content="info@kimiaomran.ae" link="mailto:info@kimiaomran.ae" />
            <ContactInfoCard index={1} icon={<Icons.Phone />} title="PHONE" content="+971 (0) 4-000-0000" link="tel:+97140000000" />
            <ContactInfoCard index={2} icon={<Icons.Location />} title="LOCATION" content="Dubai, United Arab Emirates" />
            <ContactInfoCard index={3} icon={<Icons.Clock />} title="HOURS" content="Sunday – Thursday: 8:00 AM – 6:00 PM" />
          </div>
            ...
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              <div className="bg-white dark:bg-[#011b2b] border border-gray-200 dark:border-[#064e78] rounded-sm p-6 md:p-8">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <FormInput label="FULL NAME" type="text" id="name" placeholder="Enter your full name" required value={formData.name} onChange={handleChange} />
                  <FormInput label="EMAIL ADDRESS" type="email" id="email" placeholder="Enter your email" required value={formData.email} onChange={handleChange} />
                  <FormInput label="SUBJECT" type="text" id="subject" placeholder="What is this regarding?" required value={formData.subject} onChange={handleChange} />
                  <FormInput label="MESSAGE" type="text" id="message" placeholder="Tell us about your project..." required textarea value={formData.message} onChange={handleChange} />
                  <motion.button
                    type="submit"
                  ...
                  >
                    <div className="absolute -inset-1 rounded-sm opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur" style={{ background: colors.gradient }} />
                    <div className="relative px-4 py-3 text-white rounded-sm font-semibold text-sm flex items-center justify-center gap-2" style={{ background: colors.gradient }}>
                      {isSubmitting ? (
                        <>
                ...
                <div className="flex gap-4">
                  {[
                    ...
                    <motion.a
                      ...
                      style={{ background: `${colors.quinary}10`, color: colors.quinary }}
                    >
                      {social.icon}
                    </motion.a>
                  ))}
